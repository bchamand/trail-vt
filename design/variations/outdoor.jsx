    ...
    <div ref={rootRef} style={{ width: '100%', height: '100%', overflow: 'auto', background: '#0f1412', color: '#e8e3d3', fontFamily: '"Geist", sans-serif', position: 'relative' }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 10 : 14 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1 }}>
            <span style={{ fontSize: isMobile ? 12 : 14, fontWeight: 600, letterSpacing: '0.02em' }}>TRAIL DES TECTOSAGES</span>
            {!isMobile && <span style={{ fontSize: 10, letterSpacing: '0.22em', opacity: 0.5, fontFamily: '"JetBrains Mono", monospace', marginTop: 3 }}>VIEILLE-TOULOUSE · MMXXVI</span>}
          </div>
        </div>
        {isMobile ? (
      ...
      <section style={{ position: 'relative', height: isMobile ? '88vh' : 'calc(100vh - 66px)', minHeight: isMobile ? 560 : 640, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: isMobile ? '0 20px 40px' : `0 ${pad}px 64px` }}>
          <div style={{ fontSize: isMobile ? 9 : 10, letterSpacing: '0.28em', fontFamily: '"JetBrains Mono", monospace', color: '#d4a574', marginBottom: isMobile ? 16 : 24, lineHeight: 1.5 }}>
            {isMobile ? '— SAM. 3 OCT. 2026 · 15H45' : '— SAMEDI 3 OCTOBRE 2026 · COUCHER DU SOLEIL 19H32'}
          </div>
          <h1 style={{ fontFamily: '"Instrument Serif", serif', fontSize: heroTitle, lineHeight: 0.88, fontWeight: 400, letterSpacing: '-0.035em', margin: 0 }}>
            Sur la trace<br/>
            des <span style={{ fontStyle: 'italic', color: '#d4a574' }}>Volques<br/>Tectosages.</span>
          </h1>
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'flex-end', gap: 20, marginTop: isMobile ? 28 : 48 }}>
            <p style={{ fontSize: isMobile ? 14 : 16, maxWidth: 460, lineHeight: 1.5, opacity: 0.85, margin: 0, textWrap: 'pretty' }}>
              Vieille-Toulouse fut la capitale des Volques Tectosages il y a 2300 ans. Cinq épreuves, une forêt protégée, un territoire à reconquérir avant la nuit.
            </p>
            {isMobile ? (
      ...
      <section style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', borderTop: '1px solid rgba(232,227,211,0.12)', borderBottom: '1px solid rgba(232,227,211,0.12)' }}>
        {[['V', 'ÉPREUVES'], ['30KM', 'DÉFI MAX'], ['1100', 'D+ MÈTRES'], ['19:32', 'COUCHER SOLEIL']].map(([big, small], i) => (
          <div key={i} style={{
